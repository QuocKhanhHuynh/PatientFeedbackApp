...
{
    public partial class FeedbackTypeUserControl : UserControl
    {
        private readonly MainForm mainForm;
        public FeedbackTypeUserControl(MainForm mainForm)
        {
            InitializeComponent();
        ...
        {
            var feedbackType = Service.feedbackTypeService.GetFeedbackTypes(keyword).OrderBy(x => x.Id).ToList();
            this.dtList.DataSource = feedbackType;
            this.dtList.Columns[0].HeaderText = "Mã loại khảo sát";
            this.dtList.Columns[1].HeaderText = "Tên loại khảo sát";
        ...
        private void btSave_Click(object sender, EventArgs e)
        {
            if (flag == 1)
            {
                ...
                if (id == null || id == "")
                {
                    MessageBox.Show("Mã loại khảo sát không được rỗng!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                if (name == null || name == "")
                {
                    MessageBox.Show("Tên loại khảo sát không được rỗng!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                var data = new FeedbackTypeCreateModel()
                {
                    Id = id,
                    Name = name
                };
                var result = Service.feedbackTypeService.CreateFeedType(data);
                if (!result.Status)
                {
                ...
                this.tbName.Text = "";
                this.updateList();
                MessageBox.Show("Tạo loại khảo sát mới thành công", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            if (flag == 2)
            {
                var id = this.tbId.Text.Trim();
                ...
                var data = new FeedbackTypeUpdateModel()
                {
                    Id = id,
                    Name = name
                };
                var result = Service.feedbackTypeService.UpdateFeedbackType(data);
                if (!result.Status)
                {
                    MessageBox.Show(result.Message, "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                ...
                //this.updateList();
                MessageBox.Show("Chỉnh loại khảo sát thành công", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void btSkip_Click_1(object sender, EventArgs e)
        {
        ...
        private void dtList_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            var feedbackType = Service.feedbackTypeService.GetFeedbackTypeById(id);
            ...
            this.tbId.Text = feedbackType.Id;
            this.tbName.Text = feedbackType.Name;
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                var result = Service.feedbackTypeService.DeleteFeedbackType(id);
                if (!result.Status)
                {
                    MessageBox.Show(result.Message, "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                ...
                this.rowIndex = -1;
                this.updateList();
                MessageBox.Show("Xóa loại khảo sát thành công", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                this.btSkip.PerformClick();
            }
        }
